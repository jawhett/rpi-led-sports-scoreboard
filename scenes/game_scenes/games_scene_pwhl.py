...
class PWHLGamesScene(GamesScene):
    ...
    def get_final_period_str(self, game):
        if game.get('period_type') == 'SO':
            ...
            return f"{game.get('period_num', 4) - 3}OT"
        return ""

    def build_game_in_progress_image(self, game):
        """ Builds a unified stadium-style scoreboard image for live PWHL games in progress.
        """
        ...
        # Center Info (cols 22..41, rows 0..21): Period & Clock
        clock_str = ""
        if game.get('is_intermission'):
            clock_str = "INT"
        else:
        ...
        elif game.get('period_type') == 'SO': period_str = "SO"
        elif game.get('period_type') == 'OT1': period_str = "OT"
        elif 'OT' in game.get('period_type', ''): period_str = f"{game.get('period_num', 4) - 3}OT"
